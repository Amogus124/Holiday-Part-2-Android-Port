	...
	**/
	...
	function loadAudioBuffer() {
		...
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'))) {
			...
			var leVocals:String = Paths.(currentSongName + '/Inst.' + Paths.SOUND_EXT, SOUND, 'songs');
		...
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'))) {
			...
			var leVocals:String = Paths.(currentSongName + '/Voices.' + Paths.SOUND_EXT, SOUND, 'songs');
	...
	private function saveLevel()
	{
		...
		{
                        #if android
                        saveContent(Paths.formatToSongPath(_song.song), ".json", data.trim());
                        #else
	...
	private function saveEvents()
	{
		...
		{
                        #if android
                        saveContent("events", ".json", data.trim());
                        #else
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE, onSaveComplete);
			_file.addEventListener(Event.CANCEL, onSaveCancel);
